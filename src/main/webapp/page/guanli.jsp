...
<DIV id=header class=wrap>
  ...
  <DIV class=search> 【欢迎:${user.name}】   <LABEL class="ui-green searchs"><a href="goFabu" title="">发布房屋信息</a></LABEL> <LABEL class="ui-green searchs"><a href="selectAllHousesByUid" title="">管理房屋信息</a></LABEL>
  ...
  <DIV id=houseArea>
    <TABLE class=house-list>
      ...
      <A href="details.jsp" target="_blank">
        <img src="http://localhost:82/${h.path}" width="100" height="75" alt=""></A></SPAN></TD>
          <TD>
            <DL>
              <DT><A href="details.jsp" target="_blank">${h.title}</A></DT>
              <DD>${h.dname}=${h.sname},${h.floorage}平米,户型:${h.tname}<BR>联系方式:${h.contact} </DD></DL></TD>
          <TD class=house-type><LABEL class=ui-green><INPUT onclick="location.href='selectHouse?id=${h.id}';" value="修    改" type=button name=search></LABEL></TD>
    ...
    <UL>
      <c:forEach  begin="1" end="${pageInfo.pages}" step="1" var="i">
        <LI class=current><A id=widget_338868862
                             href="selectAllHousesByUid?page=${i}"
